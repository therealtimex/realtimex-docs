...
For image attachments, RealTimeX can also normalize some unsupported source formats before provider submission. In current builds, that includes automatic HEIC or HEIF to JPEG conversion when needed for downstream model compatibility.
...
For the full preset management and usage guide, see [Slash Commands](/slash-commands).
...
If the linked runtime requests approval for a write, command, or other supervised tool action, that approval event can also appear inline in the chat history so you can review or resolve it without leaving the thread.
...
For the full goal workflow, see [Goals & Ambient Dashboard](/goals).
