---
title: "MCP on RealTimeX Docker"
description: "How to use and debug MCP tools on RealTimeX Docker"
---

import { Callout } from "nextra/components";

<Callout type="error" emoji="️‼️">
  The use of MCP for RealTimeX Docker is **self-hosting only** and is not available in the RealTimeX Cloud service.
</Callout>

## Things to know about MCP on RealTimeX Docker
...
RealTimeX Docker supports `Tools` loading via MCP Servers. We **do not** support Resources, Prompts, or Sampling.
...
RealTimeX _does not_ automatically start MCP servers when the container starts to prevent any overloading of resources on boot.
...
The majority of commands that are required to run a MCP server are available in the RealTimeX Docker container already.

The base image of RealTimeX Docker is `ubuntu:jammy-20240627.1`, so generic Ubuntu commands will be available as the user running the services inside of the container.
...
The MCP Server configuration file is located in the `plugins/RealTimeX_mcp_servers.json` file in the RealTimeX storage directory.

<Callout type="info" emoji="💡">
  The configuration file is automatically created if you open the "Agent Skills" page in the RealTimeX UI.
</Callout>

The storage directory is defined by the `STORAGE_LOCATION` environment variable when you start the RealTimeX Docker container - [see example](/installation-docker/local-docker)
...
This does not require you to restart the RealTimeX Docker container - the currently running MCPs will be killed and rebooted with whatever changes you made to the configuration file.
...
This does not require you to restart the RealTimeX Docker container - the target MCP Server will be started or stopped immediately.
...
Adding an MCP Server is as simple as adding a new tool to the `mcpServers` object in the `RealTimeX_mcp_servers.json` file in your RealTimeX storage directory.
...
This path will then be using the `STORAGE_LOCATION` directory that you defined when you [started the RealTimeX Docker container](/installation-docker/local-docker). From here you can then write and read files to the host machine.
