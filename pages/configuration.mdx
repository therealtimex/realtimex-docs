---
title: "Configuration"
description: "Other settings, environment variables, and configurations for RealTimeX"
---
...
**Desktop:**
If you are using RealTimeX Desktop, do not edit the `.env` file. This guide is only for users who are using RealTimeX Self-hosted or Docker.

</Callout>

# Configuration of RealTimeX
...
<Callout type="info" emoji="️💡">
  **Tip:** After you set these environment variables, you will need to restart
  the RealTimeX service or container for the changes to take effect.
</Callout>
...
This blocks any user, including yourself, from viewing chat history from users using the RealTimeX chat interface **and** via external embed widgets.
...
<Callout type="error" emoji="️🚨">
  **Important:** You should use an independent API key for using this feature so
  ...
  internally facing RealTimeX instances that are not exposed to the public
...
This feature is most useful for when you have RealTimeX as a simple sub-service within a much larger system and you want to leverage existing user authentication flows within that system and want to provide a seamless login experience for your users to your RealTimeX instance.
...
- You should provision an API key for RealTimeX so you can create new users as well as issue temporary authentication links for users.
- The user must already exist within RealTimeX before using this feature. You can create a user via the in-app interface or the API.
...
Once enabled, you can issue a temporary authentication link for a user leveraging the `/v1/users/{id}/issue-auth-token` endpoint via the RealTimeX API.
You simply need to provide the user ID and the API key you created earlier to generate a temporary authentication token that can be used by the target user to login to RealTimeX.

```bash copy
curl -X POST "https://your-RealTimeX-instance.com/v1/users/{id}/issue-auth-token" \
...
Now, the user can visit the provided `loginPath` URL in their browser to be automatically logged in to RealTimeX!

```text copy
https://your-RealTimeX-instance.com/sso/simple?token=1234567890
```
...
The user will be redirected to the home page of RealTimeX after logging in.
...
```text copy
https://your-RealTimeX-instance.com/sso/simple?token=1234567890&redirectTo=/workspaces/sample-workspace
```
...
## RealTimeX Hub Agent Skills

<Callout type="error" emoji="️🚨">
  **Important:** Agent skills can enable running untrusted code from untrusted
  sources. By default, RealTimeX will not allow downloading agent skills from
  the RealTimeX Hub.
</Callout>

Modification of the `COMMUNITY_HUB_BUNDLE_DOWNLOADS_ENABLED` environment variable allows you to pull in agent skills from the RealTimeX Hub.
...
- `COMMUNITY_HUB_BUNDLE_DOWNLOADS_ENABLED=1`: This configures enables RealTimeX to download agent skills from the RealTimeX Hub but **only if the item is verified or a private item**.
- `COMMUNITY_HUB_BUNDLE_DOWNLOADS_ENABLED=allow_all`: This configures enables RealTimeX to download agent skills - including unverified public items - from the RealTimeX Hub.
...
By default, the collector does not allow scraping of [local IP addresses](https://github.com/therealtimex/RealTimeX/blob/master/collector/utils/url/index.js#L24).
...
By default, RealTimeX attempts to use streaming for a better user experience. However, some custom LLM implementations may not support this feature, resulting in errors or unexpected behavior.
