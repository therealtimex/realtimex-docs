...
- working directories
- [Ambient Agent](/ambient-agent) scheduling and behavior
...
- `Workspace Settings > Chat` for the active workspace `System Prompt`
- `Workspace Settings > Personality` for file-based workspace identity and instruction files
- [Slash Commands](/slash-commands) for reusable manually-invoked prompt presets

For the reusable preset libraries themselves, see [System Prompts](/system-prompts) and [Personality](/personality).
...
- Use [Credentials](/credentials) when workflows need reusable outbound secrets.
- Use [Agent Authentication](/agent-authentication) when launched runtimes such as `Claude Code`, `Codex CLI`, `Qwen`, or `Ambient Agent` need provider API keys injected by RealTimeX.

For `Antigravity CLI` and `Cursor Agent`, use the runtime's own login flow first, then verify readiness from [Agent Runtime](/agent-runtime) instead of expecting an `Agent Authentication` profile.
...
- behavior layer reviewed if needed
