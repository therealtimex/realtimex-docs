...
**This guide is intended for developers who want to create custom agent skills for RealTimeX.**
...
1. NodeJS 18+
2. Yarn
3. RealTimeX running in some supported environment see [here](./introduction.mdx) for more information.
...
RealTimeX supports hot loading of custom agent skills. This means that you can make changes to your custom agent skill and see the changes without having to restart the agent or the instance of RealTimeX.

## Where to place your custom agent skill code

All agents skills must be placed in the appropriate folder in your RealTimeX storage directory folder. This can be found in multiple locations depending on the environment you are running RealTimeX in.
...
When running RealTimeX locally, your storage directory is likely mounted in the `server/storage` directory.
...
When running RealTimeX on Desktop, your storage directory can be [found using this guide](../../installation-desktop/storage.mdx#where-is-my-data-located).
