...
RealTimeX allows you to create custom agent skills that can be used to extend the capabilities of your `@agent` invocations. These skills can be anything you want from a simple API call to even operating-system invocations.

The sky is the limit! Depending on how you run RealTimeX, you can create custom agent skills that can run extra processes like running a local Python script or, on Desktop, even operating-system invocations.
...
<Callout type="info" emoji="️💡">
 Custom agent skills are newly supported in RealTimeX and may have some bugs, quirks, missing features, unsupported features, etc.
...
Custom agent skills are available in the Docker image since [commit `d1103e`](https://github.com/therealtimex/RealTimeX/commit/d1103e2b71ae5550fa33d7d74be5fe3e35e6b1b1) or [release v1.2.2](https://github.com/therealtimex/RealTimeX/releases/tag/v1.2.2).

Custom agent skills are available in RealTimeX Desktop version **1.6.5 and later.**

Custom agent skills are **not** available in the RealTimeX Cloud offering.

## View loaded custom agent skills

You can view the loaded custom agent skills by opening the `Agent Skills` tab in the settings of RealTimeX.

Any valid custom agent skills loaded into RealTimeX will be displayed here.
