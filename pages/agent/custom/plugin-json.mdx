...
description: "Understand the plugin.json file and how to use it to define custom agent skills for RealTimeX."
---

**This page is intended for developers who want to create custom agent skills for RealTimeX.**

# `plugin.json` reference

The `plugin.json` file is used to define a custom agent skill for RealTimeX. It is a **JSON** file that contains the following properties:
...
The `name` property is a string that is used to identify the custom agent skill. This is the human-readable name of the skill that is displayed in the RealTimeX UI.
...
The key of the setup argument is the name of the argument that is used in the handler.js file, while its properties automatically generate the UI and inputs for the argument in the RealTimeX UI.
...
"setup_args": {
    "OPEN_METEO_API_KEY": {
      ...
      // Defines the UI of the input to be rendered in the RealTimeX UI
      "input": {
      ...
      "value": "" // (optional) preset value of the argument - will be replaced by the user input in the RealTimeX UI, but can be hardcoded.
    }
  },
```
