...
<Callout type="info" emoji="️💡">
  RealTimeX can have varying default variables depending on if you are using the **RealTimeX via Docker** or **RealTimeX Desktop** version.
</Callout>

RealTimeX comes with a set of default variables that you can use in your system prompt. You can view the full list of active variables by clicking on the **System Prompt Variables** link in the sidebar under **Tools** when on the settings page.
...
<Image
  ...
  width={1920}
  quality={100}
  alt="RealTimeX System Prompt Variables"
...
| `{user.name}` | The name of the user | RealTimeX Docker (with multi-user mode enabled) |
| `{user.bio}` | The bio field of the user | RealTimeX Docker (with multi-user mode enabled) |
| `{os.name}` | The name of the operating system | RealTimeX Desktop |
| `{os.arch}` | The architecture of the operating system | RealTimeX Desktop |

_Note: Any time based variable will the current time **of the machine** RealTimeX is running on. Keep this in mind in Docker based versions of RealTimeX._
...
<Image
  ...
  width={1920}
  quality={100}
  alt="RealTimeX Custom Variables"
/>
