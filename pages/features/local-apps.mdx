...
#### Compatible Mode (Recommended for beginners)
- **Low complexity** - Just run SQL scripts to set up
...
- `claim_task_compatible()` - Atomically claim a task with lock
...
If you prefer to run SQL manually or the Auto-Setup fails, you can view the complete SQL scripts by clicking **View Setup** in the Compatible Mode configuration panel.
...
<Callout type="warning">
**Retry Limit**: After 3 failed attempts from different machines, consider investigating the root cause. The task may have data issues.
</Callout>
...
**Solution**:
- Ensure your lock logic uses atomic operations
...
- [Agent Flows](/agent-flows/overview) - Build complex workflows with Agent Flows
