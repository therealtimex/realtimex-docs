...
The <b>Automatic Document Sync</b> feature for RealTimeX allows you to "watch" a document for active changes. When changes are detected the file will be re-embed and all workspaces using this file will automatically be updated.
...
Watching a file with RealTimeX's Automatic Document Sync will periodically fetch and replace all embeddings of that document across all of your active workspace.
...
If you are having issue with the document sync feature simply disable the toggle for the feature and it will not run any background workers while using RealTimeX or on reboots.
...
_Reminder:_ this is only available on RealTimeX Desktop

</Callout>

On RealTimeX Desktop you can now "watch" any locally uploaded file! Functionally, this works the exact same as watching content that comes from a website or elsewhere, but there
...
Files will be checked for new content every **10 minutes**. The app must be open for this to occur as RealTimeX does not minimize to the tray or taskbar when closed. If changes are found, the document content and all workspaces will be updated automatically.
...
RealTimeX cannot and does not know where a file is relocated should you move it. On the next interval sync the document will be marked as "Not Found" and it will
