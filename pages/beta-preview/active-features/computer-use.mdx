...
_The following risks are relevant to any AI using your computer, not just RealTimeX_
...
The <b>Computer use</b> feature for RealTimeX is an experimental feature that allows you to enable an AI to use your computer to complete tasks.
...
- **Accessibility:** (MacOS only) This feature requires the `Accessibility` and `Screen Recording` permissions to be enabled for RealTimeX.
...
You can do this by clicking the pause icon in the UI, pressing `CMD+K` or `CTRL+K`, or by quitting the RealTimeX application.
...
_This section is relevant to users running RealTimeX Desktop on MacOS_
...
In order to use the computer use feature, you need to have the `Accessibility` permissions enabled for RealTimeX on your system.

This is done by opening the `Security & Privacy` settings on MacOS and clicking on the `Privacy` tab. From there, find `Accessibility` on the left and click on the `+` button to add RealTimeX.

This will allow RealTimeX to control your computer's mouse and keyboard.
...
In order to use the computer use feature, you need to have the `Screen Recording` permissions enabled for RealTimeX on your system.

This is done by opening the `Security & Privacy` settings on MacOS and clicking on the `Privacy` tab. From there, find `Screen Recording` on the left and click on the `+` button to add RealTimeX.

This will allow RealTimeX to take screenshots of your display.
...
<Callout type="info">
  **Note:** Be ready at any time to abort the computer use session if things are
  ...
  RealTimeX application.
</Callout>

Once you have enabled the feature and configured it with your API key, you can invoke computer use by typing in `@os` in the RealTimeX chat along with a prompt.
...
Once the OS agent is running, RealTimeX will minimize to get out of the way and you should see a popup in your display allowing you to control or halt the OS agent.
...
You can also quit the RealTimeX application which will halt the OS agent as well. You can drag the popup around to get it out of the way, but this may interfere with the OS agent's ability to control your mouse position if needed.
...
The OS agent will output its actions and any relevant information to the RealTimeX chat as it executes. These actions are currently **not** saved or stored your workspace's chat history.
