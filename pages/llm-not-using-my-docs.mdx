...
<Callout type="info" emoji="️💡">
  **Tip:**
    ...
    Luckily, it's simple to solve for, actually! These ideas extend beyond just RealTimeX.
...
So to understand why this occurs we first need to clear up some confusion on how RAG (retrieval augmented generation) works inside of RealTimeX.
...
In RealTimeX, we do not read your entire filesystem and then report that to the LLM, as it would waste tokens 99% of the time.
...
If this is what you want, you are thinking of agents, which are coming to RealTimeX soon.

## So how does RealTimeX work?

Let's think of RealTimeX as a framework or pipeline.
...
7. The vector database then calculates the "nearest" chunk-vector. RealTimeX filters any "low-score" text chunks (you can modify this). Each vector has the original text it was derived from attached to it.
...
<Callout type="warning" emoji="️⚠️">
  **IMPORTANT!**
    ...
    However, here is where the embedder model used and other RealTimeX settings can make the most difference. Read more in the next section.
...
RealTimeX exposes many many options to tune your workspace to better fit with your selection of LLM, embedder, and vector database.

If you need to adjust how documents are segmented before embeddings are created, see [Text Splitting & Chunking](/text-splitting-chunking) for the current system-wide chunking controls.

The workspace options are the easiest to mess with and you should start there first. RealTimeX makes some default assumptions in each workspace. These work for some but certainly not all use cases.
...
<Image
  ...
  width={1920}
  quality={100}
  alt="RealTimeX Workspace settings"
/>
...
<Image
  ...
  width={1920}
  quality={100}
  alt="RealTimeX Workspace RAG Search Preference"
/>

By default, RealTimeX will search for the most relevant chunks of text. For the majority of use cases this is the best option since it is very simple to run and very fast to calculate.
...
RealTimeX will trim data from the context if you are going to overflow the model - which will crash it. So it's best to keep this value anywhere from 4-6 for the majority of models. If using a large-context model like Claude-3, you can go higher but beware that too much "noise" in the context may mislead the LLM in response generation.
...
<Image
  ...
  width={1920}
  quality={100}
  alt="RealTimeX Document Pinning"
/>
