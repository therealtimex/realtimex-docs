...
**Response (SSE Events):**
- `data`: JSON object with `type` field.
  - `type: "agentThought"`: Internal reasoning or tool execution status. Content in `thought` field.
  - `type: "textResponse"`: The final response content. Content in `textResponse` field.
...
**Response (SSE):**
- `event: info`: Initial info with chunk count.
- `event: chunk`: Audio chunk in base64.
