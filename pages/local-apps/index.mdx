...
<Callout type="info">
Local Apps requires a current **RealTimeX Desktop** build with Local Apps support enabled.
</Callout>
...
- **LLM Proxy & RAG** - Access configured LLM providers and managed Vector Storage
...
- **Standard table structure** - Uses the predefined `rtx_activities` table
...
RealTimeX uses a **manifest-based permission system** to ensure security. 

- **Production Mode**: Uses `x-app-id` with manifest-declared permissions.
- **Developer Mode**: Uses **API Keys** for full access during local development (no manifest required).

#### How It Works

1.  **Declaration**: You define required permissions (e.g., `['api.agents', 'webhook.trigger']`) when initializing the `RealtimeXSDK`.
2.  **Upfront Prompt**: When the app starts, RealTimeX checks if these permissions are granted. If any are missing, a **Bulk Permission Modal** appears, allowing the user to grant them all at once.
3.  **Runtime Prompt**: If your app calls an API for a permission not declared upfront (or if the user skipped the upfront prompt), the SDK will automatically trigger a **Native Permission Dialog** and re-attempt the API call once granted.
...
| `api.threads` | List threads in workspaces |
| `api.task` | Get task execution status |
| `webhook.trigger` | Trigger AI agents via webhook |
| `database.config` | Read Supabase configuration |
| `activities.read` | Read activities data |
| `activities.write` | Create, update, delete activities |
| `llm.chat` | Access LLM chat completion (Sync/Stream) |
| `llm.embed` | Generate text embeddings |
| `llm.providers` | List available LLM models and providers |
| `vectors.read` | Query managed vector storage |
| `vectors.write` | Upsert or delete vectors in storage |

#### Managing Permissions

- **Grant**: Upfront modal or native runtime dialogs.
- **Reset**: Click app menu (⋮) → **Reset Permissions** to revoke all permissions.
- **Individual Control**: Manage specific permissions in the **Permissions** tab of the Local App detail page.
...
- **RealTimeX Desktop** with Local Apps support
...
**`rtx_activities` Table:**

| Column | Type | Description |
|--------|------|-------------|
| `id` | UUID | Primary key, auto-generated |
| `user_id` | UUID | Maps to auth.users (RLS protected) |
...
- `rtx_fn_claim_task()` - Securely claim a task with lock
- `rtx_fn_complete_task()` - Mark task as completed with result
- `rtx_fn_fail_task()` - Mark task as failed, release lock
...
#### Retry Broadcast Implementation

When a task fails and you want other machines to retry, broadcast via Supabase Realtime:

```javascript
const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);

// Broadcast retry signal
await supabase.channel('task-retry-YOUR_APP_NAME')
  .send({
    type: 'broadcast',
    event: 'task-available',
    payload: {
      task_id: 'uuid',
      table_name: 'rtx_activities',
      failed_by: 'machine_id_that_failed',
      retry_count: 1
    }
  });

// Other Electron clients listening to this channel will
// attempt to claim the task via their configured lock_url
```

#### Stale Lock Recovery (Critical)

<Callout type="warning">
**This is CRITICAL for Custom Mode!** When RealTimeX crashes, tasks remain locked indefinitely. You MUST implement stale lock recovery.
</Callout>
...
-- Step 3: Create the stale lock recovery function
CREATE OR REPLACE FUNCTION public.recover_stale_locks_custom(
  table_name TEXT DEFAULT 'rtx_activities',
  timeout_minutes INT DEFAULT 5
)
RETURNS TABLE(recovered_count INT, recovered_ids UUID[]) AS $$
DECLARE
  affected_ids UUID[];
  affected_count INT;
BEGIN
...
-- Step 4: Schedule automatic recovery every minute
SELECT cron.schedule(
  'recover-stale-locks-custom',
  '* * * * *',
  $$SELECT public.recover_stale_locks_custom('your_table_name', 5);$$
);
...
**Option 2: Local App Implementation**

If you cannot use pg_cron, implement recovery in your Local App server:
...
async function recoverStaleLocks() {
  try {
    ...
    if (findError || !staleTasks?.length) return;

    console.log(`[StaleLockRecovery] Found ${staleTasks.length} stale tasks`);
    ...
    for (const task of staleTasks) {
      ...
      await channel.send({
        ...
        payload: {
          ...
          failed_by: task.locked_by,
          recovery_reason: 'stale_lock',
        },
      });
    }
  } catch (error) {
    console.error('[StaleLockRecovery] Error:', error);
  }
}

// Start the recovery loop
function startStaleLockRecovery() {
  recoverStaleLocks();
  setInterval(recoverStaleLocks, CHECK_INTERVAL_MS);
}

// Call this when your Local App server starts
startStaleLockRecovery();
```

#### Get Publications RPC (Required)

This function is required for RealTimeX to discover which tables are enabled for Realtime:
...
SELECT json_agg(pub_row ORDER BY pubname)
FROM (
  SELECT
    pubname,
    system_id,
    ...
    json_agg(
      DISTINCT jsonb_build_object(
        'schema', COALESCE(parent_schema, child_schema),
        'table', COALESCE(parent_table, child_table)
      )
    ) FILTER (WHERE child_table IS NOT NULL) AS tables
...
```json
{
  "config": {
    "supabase": {
      "tables": ["public.rtx_activities", "public.orders"],
      "tableActions": {
        "public.rtx_activities": ["INSERT", "UPDATE"],
