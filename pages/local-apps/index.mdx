...
When a Local App starts for the first time, users are prompted to grant permissions. This provides granular control over what your app can access.
...
- **Grant**: Permissions modal appears on first app launch
- **Reset**: Click app menu (⋮) → **Reset Permissions** to revoke all permissions
- **Re-grant**: Next app start will prompt for permissions again

<Callout type="info">
Permissions are stored locally per-app and are **not** included when publishing apps to the marketplace.
</Callout>
