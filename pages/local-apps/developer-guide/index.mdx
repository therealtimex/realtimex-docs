...
<Tabs items={['TypeScript', 'Python']}>
  ...
  <Tabs.Tab>
    ...
    print(f"Using Supabase URL: {config.url}")
    ...
    if response.session and response.session.access_token:
        sync_result = await sdk.auth.sync_supabase_token(response.session.access_token)
        print(f"Token synced successfully: {sync_result.success}")
    ```
  </Tabs.Tab>
</Tabs>
...
<Tabs items={['TypeScript', 'Python']}>
  ...
  <Tabs.Tab>
    ```python
    providers = await sdk.stt.list_providers()
    # providers: List[dict] with id, name, models
...
<Tabs items={['TypeScript', 'Python']}>
  ...
  <Tabs.Tab>
    ```python
    # List all servers (local + remote)
    servers = await sdk.mcp.get_servers()

    # Filter by provider
    local_only = await sdk.mcp.get_servers("local")
    remote_only = await sdk.mcp.get_servers("remote")

    for server in servers:
        print(f"{server['name']} ({server['provider']}) - {server['display_name']}")
    ```
  </Tabs.Tab>
</Tabs>
...
<Tabs items={['TypeScript', 'Python']}>
  ...
  <Tabs.Tab>
    ```python
    tools = await sdk.mcp.get_tools("fetch")
...
<Tabs items={['TypeScript', 'Python']}>
  ...
  <Tabs.Tab>
    ```python
    # Execute the 'fetch' tool on the local 'fetch' MCP server
    result = await sdk.mcp.execute_tool("fetch", "fetch", {
        "url": "https://httpbin.org/get",
        "max_length": 500
    })
    ...
    # Execute on a remote server
    remote_result = await sdk.mcp.execute_tool(
        "github", "list-repos", {"user": "octocat"}, provider="remote"
    )
    ```
  </Tabs.Tab>
</Tabs>
