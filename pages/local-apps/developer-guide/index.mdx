...
<Tabs items={['TypeScript', 'Python']}>
  ...
  <Tabs.Tab>
    ...
    # OR Developer Mode (API Key)
    sdk_dev = RealtimeXSDK(config=SDKConfig(
...
<Tabs items={['TypeScript', 'Python']}>
  ...
  <Tabs.Tab>
    ```python
    # Create a new session
    session = await sdk.agent.create_session(
        agent_name="@agent",
        workspace_slug="operations"
    )
    ...
    # Get session info
    info = await sdk.agent.get_session(session.session_id)

    # Close session
    await sdk.agent.close_session(session.session_id)
    ```
  </Tabs.Tab>
</Tabs>
...
<Tabs items={['TypeScript', 'Python']}>
  ...
  <Tabs.Tab>
    ```python
    response = await sdk.agent.chat(session.session_id, "Analyze this dataset")
    print(response.text)
...
<Tabs items={['TypeScript', 'Python']}>
  ...
  <Tabs.Tab>
    ```python
    async for event in sdk.agent.stream_chat(session.session_id, "Analyze this dataset"):
        if event.type == 'agentThought':
            print(f"Thinking: {event.thought}")
        elif event.type == 'textResponse':
            print(event.textResponse, end="", flush=True)
        elif event.type == 'responseData':
            # Handle rich blocks
            import json
            block = json.loads(event.textResponse)
            print(f"Rich Block: {block['dataType']}")
    ```
  </Tabs.Tab>
</Tabs>
...
<Tabs items={['TypeScript', 'Python']}>
  ...
  <Tabs.Tab>
    ```python
    response = await sdk.agent.start_chat("Quick question", agent_name="@agent")
    ```
  </Tabs.Tab>
</Tabs>
...
<Tabs items={['TypeScript', 'Python']}>
  <Tabs.Tab>
    ...
    console.log('My persistent storage is at:', dataDir);
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```python
    data_dir = await sdk.get_app_data_dir()
    print(f"My persistent storage is at: {data_dir}")
    ```
  </Tabs.Tab>
</Tabs>

<Callout type="info">
**Development Mode:** In development mode using an API Key, the directory is derived from the masked API key to prevent data collision between different development environments.
</Callout>
...
<Tabs items={['TypeScript', 'Python']}>
  <Tabs.Tab>
    ...
    // providers: [{ name, label, description, fields: [...] }]
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```python
    res = await sdk.llm.vectors.list_providers()
    # res.providers: List[VectorProviderMetadata]
    ```
  </Tabs.Tab>
</Tabs>

#### 2. Register Custom Configuration
...
<Tabs items={['TypeScript', 'Python']}>
  ...
  <Tabs.Tab>
    ```python
    await sdk.llm.vectors.register_config('pgvector', {
        "databaseUrl": "postgresql://user:pass@host:5432/db",
        "tableName": "my_app_vectors"
    })
    ```
...
<Tabs items={['TypeScript', 'Python']}>
  <Tabs.Tab>
    ...
    console.log(`Using ${provider} at ${config.uri || config.databaseUrl}`);
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```python
    res = await sdk.llm.vectors.get_config()
    print(f"Using {res.provider}")
    ```
  </Tabs.Tab>
</Tabs>

<Callout type="info">
**Persistence:** Once a custom configuration is successfully registered, RealtimeX persists it for your App ID. All subsequent `upsert`, `query`, and `search` calls will automatically use this backend.
</Callout>
...
<Tabs items={['TypeScript', 'Python']}>
  ...
  <Tabs.Tab>
    ...
    # List[dict] with id, name, type, configured, config
    
    # Filter to configured providers only
    available = [p for p in providers if p.get('configured')]
    ```
  </Tabs.Tab>
</Tabs>

#### Generate Speech (Buffer)
...
<Tabs items={['TypeScript', 'Python']}>
  ...
  <Tabs.Tab>
    ...
    # audio_bytes is bytes, save to file
...
<Tabs items={['TypeScript', 'Python']}>
  ...
  <Tabs.Tab>
    ```python
    async for chunk in sdk.tts.speak_stream("Long text content..."):
        print(f"Chunk {chunk['index'] + 1}/{chunk['total']}")
        
        # chunk['audio'] is bytes (already decoded!)
        audio_bytes = chunk['audio']
        mime_type = chunk['mimeType']
        
        # Save or play each chunk
        with open(f"chunk_{chunk['index']}.wav", "wb") as f:
            f.write(audio_bytes)
    ```
  </Tabs.Tab>
</Tabs>
...
Convert spoken audio into text using local (Whisper, Native) or cloud (Groq) providers.
...
Retrieve a list of models that are currently downloaded and ready for use. This helps precise model selection for the `native` or `whisper` provider.

<Tabs items={['TypeScript', 'Python']}>
  <Tabs.Tab>
    ```typescript
    const models = await sdk.stt.models();
    // models: [{ id: 'ggml-base.en', name: 'Whisper Base (English)', provider: 'native', ... }]
  ...
  <Tabs.Tab>
    ```python
    models = await sdk.stt.models()
    # models: List[dict] with id, name, provider
...
<Tabs items={['TypeScript', 'Python']}>
  <Tabs.Tab>
    ...
    const { text, error } = await sdk.stt.listen({
      ...
      provider: 'native',          // Optional: 'native', 'whisper', 'groq'
      model: 'ggml-base.en'        // Optional: specific model ID (must be downloaded)
    });
  ...
  <Tabs.Tab>
    ```python
    response = await sdk.stt.listen(
        language="en-US",          # Optional
        timeout=10000,             # Optional: ms
        provider="native",         # Optional
        model="ggml-base.en"       # Optional
    )
    
    if response.error:
        print(f"STT failed: {response.error}")
    else:
...
<Callout type="tip">
**Providers:**
- **native**: Uses local Whisper.cpp (fastest, supports GGML models).
- **whisper**: Uses in-browser Transformers.js (supports ONNX models).
- **groq**: Uses Groq Cloud API (fastest cloud, requires API key).
</Callout>
