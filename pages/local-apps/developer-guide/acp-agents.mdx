...
description: "Use CLI-based AI agents (Claude, Cursor, Gemini, Qwen, etc.) through the SDK"
---
...
The ACP Agent module lets your Local App create interactive sessions with CLI-based AI agents like Claude Code, Cursor Agent, Gemini CLI, Qwen, and others. Agents run as persistent processes that can read files, write code, execute commands, and interact with tools.
...
<Tabs items={['TypeScript', 'Python']}>
  <Tabs.Tab>
    ...
    // ["claude (Claude Code)", "gemini (Gemini CLI)", "qwen (Qwen CLI)", "acpx-cursor (Cursor Agent)"]
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```python
...
For example, Cursor sessions use `agent_id: 'acpx-cursor'` from discovery instead of a plain `cursor` id.
