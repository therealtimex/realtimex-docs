...
description: "Use CLI-based AI agents such as Claude, Cursor, Gemini, Grok, and Qwen through the SDK"
---
...
The ACP Agent module lets your Local App create interactive sessions with CLI-based AI agents such as Claude Code, Cursor Agent, Gemini CLI, Grok CLI, Qwen CLI, and others. Agents run as persistent processes that can read files, write code, execute commands, and interact with tools.

<Callout type="warning">
`Antigravity CLI` is currently terminal-only in RealTimeX. It may appear in runtime discovery, but it is not ACP-launchable through `createSession()` and should be launched from the terminal runtime path instead.
</Callout>

<Callout type="info">
`Cursor Agent` is ACP-launchable through the synthetic provider id `acpx-cursor`. Its terminal route resolves the `agent` command first, can fall back to `cursor-agent`, and can be overridden with `CURSOR_CLI_PATH`.
</Callout>

<Callout type="info">
The exact ACP-launchable catalog changes as RealTimeX adds providers and as CLIs are installed on the host. Always call `listAgents()` instead of hard-coding a fixed provider list.
</Callout>
...
<Tabs items={['TypeScript', 'Python']}>
  <Tabs.Tab>
    ...
    const installed = agents.filter(a => a.installed && a.supported !== false);
    console.log(installed.map(a => `${a.id} (${a.label})`));
    // ["claude (Claude Code)", "gemini (Gemini CLI)", "grok-cli (Grok CLI)", "qwen (Qwen CLI)", "acpx-cursor (Cursor Agent)"]
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```python
    agents = await sdk.acp_agent.list_agents()
    installed = [a for a in agents if a.installed and getattr(a, "supported", True) is not False]
...
Use an ACP-launchable agent id here. If discovery returns an agent with `supported: false` or `terminalOnly: true`, do not pass that agent to `createSession()`.

For example, Grok sessions use the discovered `grok-cli` id when that provider is installed, while Cursor sessions use `agent_id: 'acpx-cursor'` instead of a plain `cursor` id.
...
<Tabs items={['TypeScript', 'Python']}>
  ...
  <Tabs.Tab>
    ```python
    agents = await sdk.acp_agent.list_agents()
    qwen = next(a for a in agents if a.id == "qwen-cli")
    print(qwen.supportsProviderForwarding)  # True
    print(qwen.forwardableProviders)
    # ["openai", "anthropic", "gemini", "realtimexai"]
    ```
  </Tabs.Tab>
</Tabs>
...
<Tabs items={['TypeScript', 'Python']}>
  ...
  <Tabs.Tab>
    ...
    session = await sdk.acp_agent.create_session(
        "qwen",
        forwarded_provider="realtimexai",
...
### Troubleshooting Qwen authentication mismatches

If a Qwen-backed session fails with an error like `Authentication failed: ANTHROPIC_BASE_URL environment variable not found`, the session is not using native Qwen/DashScope authentication. It is running with the forwarded provider's protocol, so the Qwen process expects that provider's environment and base URL configuration.

- Native Qwen or DashScope usage typically runs through an OpenAI-compatible endpoint and needs the matching API key plus base URL.
- If `forwardedProvider` is `anthropic`, the spawned Qwen process must receive Anthropic-compatible settings such as `ANTHROPIC_API_KEY` and `ANTHROPIC_BASE_URL`.
- If `forwardedProvider` is `openai` or another OpenAI-compatible provider, the process must receive OpenAI-compatible settings such as `OPENAI_API_KEY` and `OPENAI_BASE_URL`.

<Callout type="warning">
The auth prompt may still mention `QWEN_API_KEY` or `DASHSCOPE_API_KEY` because the CLI binary is Qwen. When provider forwarding is enabled, follow the forwarded provider's auth requirements instead. Qwen Code's official auth matrix is documented at [Qwen Code Authentication](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth/).
</Callout>
