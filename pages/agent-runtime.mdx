...
RealTimeX also supports interactive terminal agent runtimes such as `Antigravity CLI`, `Cursor Agent`, `Claude Code`, `Codex CLI`, `Gemini CLI`, `Grok CLI`, `Qwen CLI`, and `OpenCode`.
...
`Grok CLI` is ACP-launchable in current RealTimeX builds.

- RealTimeX discovers it through the `grok` or `grok-cli` handles
- authentication can come from `grok login` or an injected `XAI_API_KEY` profile
- model discovery follows the Grok CLI runtime's current available model list
- permission handling uses the standard supervised ACP flow rather than the ACPX-only `approve-all` path

### Supervised approval flows

ACP-backed terminal sessions can run with explicit approval policies.
...
When the policy does not auto-approve an action, the session can emit a `permission_request` event.

Depending on where the runtime is being consumed, that request can surface:

- in the RealTimeX chat history
- in the terminal-linked session UI
- over Local Apps ACP session streams and sync APIs

Use `approve-reads` when you want read access to flow freely but still want review over writes and higher-risk tool actions.

## Agent Authentication

`Agent Authentication` stores provider API keys that RealTimeX injects into launched agent runtimes such as `Claude Code`, `Codex CLI`, `Gemini CLI`, `Grok CLI`, `Qwen CLI`, `OpenCode`, and `Ambient Agent`.
